...
# 🚀 快速使用
## 一、下载与解压
首先获取软件安装包：点击链接 [MouseEngine-Beta1](https://github.com/quanmouren/MouseEngine/releases/download/Beta1/MouseEngine-Beta1-windows-x64.zip) 下载软件压缩包。下载完成后，将压缩包解压到你希望存放的位置。
...
## 三、配置光标组
启动解压目录中的 `MouseGroupSettings.exe` 程序
...
## 四、绑定壁纸与光标组
运行 `WallpaperListSettings.exe` 程序
...
可进行程序运行相关配置，核心选项为“自启动设置”：勾选对应选项后，程序将随系统开机自动运行；其余辅助配置（如日志级别、进程优先级等）默认适配常规使用场景，无需额外调整。
