...
**语言 / Language**: 简体中文 | [English](../README.md) | [日本語](./README_JA.md)
...
```toml
[config]
enable_default_icon_group = true  #未匹配到鼠标组时是否启用默认鼠标组。
pause_on_fullscreen = false       #检测到全屏程序时是否暂停切换。
strict_window_judgment = false    #是否使用更严格的鼠标所在窗口判定。
show_more_menu = false            #是否显示更多托盘菜单项。
language = "zh-CN"
specified_mouse_group = ""        #临时指定使用的鼠标组，留空表示不强制指定。
use_new_menu = true               #是否使用新版统一托盘菜单入口。
```
...
### Q3：为什么壁纸切换后光标没有变化？/ 光标显示错误？

- 确认 Wallpaper Engine 路径正确。
- 确认目标壁纸已经绑定鼠标组。
- 确认鼠标组中的 `.cur` / `.ani` 文件路径有效。
- 如果当前前台程序命中白名单，白名单规则会优先于壁纸规则。
- 如果你在使用多显示器,可以参考以下方案排除故障：
    - 如果你调整过windows的显示器设置,请重启计算机。
    - 如果你重新连接过显示器,请重启计算机。
    - 如果你的显示器支持双模,且在本次开机至当前时间,切换过双模状态,请重启计算机。
    - 如果你的显示器在本次开机至当前时间,曾发生过重连,请重启计算机。
    - 如果你切换过显示模式,请重启计算机。
